...
/**
 * Homepage compare teaser — a representative eight, each linking to its full
 * head-to-head page under `/compare/{slug}`. The `/compare` hub lists the whole
 * field. Kept intentionally light (no comparison table) so the homepage stays
 * scannable.
 */
const competitors = [
  ...
  { slug: "cognito", name: "AWS Cognito", category: "Platform / hyperscaler" },
  { slug: "firebase", name: "Firebase Auth", category: "Platform / hyperscaler" },
  { slug: "keycloak", name: "Keycloak", category: "Open source" },
  { slug: "zitadel", name: "Zitadel", category: "Open source" },
];
        ...
        {competitors.map((c) => (
          <StaggerItem key={c.name} className="h-full">
            <SpotlightCard className="h-full rounded-[1.75rem]">
              <Link
                href={c.slug ? `/compare/${c.slug}` : "/compare"}
